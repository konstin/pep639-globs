...
pub enum Pep639GlobError {
    ...
    #[error(
        "Glob contains pattern directory indicator (`..`) at position {pos}, which is not allowed."
    )]
...
mod tests {
    ...
    fn test_error() {
        ...
        for (input, expected) in cases {
            let actual = parse_pep639_glob(input).unwrap_err();
